...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if self.opt.random_pair:  # source and target use same dataset, same path
        ### input A (label maps of source)
            ...
            if opt.isTrain or opt.use_encoded_image:  # TODO: test时候只有encoded image才会输入图像
                dir_B = '_source_img'
        ...
        else:  # the pair is fixed, use in test mode usually
            assert not self.opt.random_pair
    ...
    def __get_set1__(self, index, s=True):
        ...
        if self.opt.label_nc == 0:
            ...
            A_tensor = transform_A(A) * 255.0  # TODO: 把label*255了，为啥不Norm？
    ...
    def __get_set2__(self, index):
        ...
        target_dict = {'t_label': D_tensor, 't_inst': E_tensor, 't_image': F_tensor, 
